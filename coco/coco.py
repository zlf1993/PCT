...
class CocoDataset(cocoutils.Dataset):
    ...
    def generator(self, image_id):
        ...
        if gt is None:
            ...
            floor_y = np.floor(gt_y/4).astype(int)
            floor_x = np.floor(gt_x/4).astype(int)
            ...
            for i in range(instance_num):
                vertical_ones = np.tile([[1]], [self.config.IMAGE_MAX_DIM//4, 1])
                # print(np.shape(vertical_ones))
                horizontal_ones = tf.tile([[1]], [1, self.config.IMAGE_MAX_DIM//4])
                pad_l = floor_x[i]
                pad_r = self.config.IMAGE_MAX_DIM//4 - floor_x[i] - 1
                vertical_pads = np.pad(vertical_ones, [[0, 0], [pad_l, pad_r]], "constant", constant_values=(gt_left[i][0]//4, gt_right[i][0]//4))
                vertical_pads = np.expand_dims(vertical_pads, -1)
                # print(np.shape(vertical_pads))
                pad_t = floor_y[i]
                pad_b = self.config.IMAGE_MAX_DIM//4 - floor_y[i] -1
                horizontal_pads = np.pad(horizontal_ones, [[pad_t, pad_b], [0, 0]], "constant", constant_values=(gt_top[i][0]//4, gt_bot[i][0]//4))
            ...
            if instance_num <= self.config.MAX_GT_INSTANCES:
                masks = np.pad(gt_mask, ((0, 0), (0, 0), (0, self.config.MAX_GT_INSTANCES - instance_num)),
                               mode='constant')
                scalar_y = np.pad(scalar_y, ((0, 0), (0, 0), (0, self.config.MAX_GT_INSTANCES - instance_num)),
                               mode='constant')
                scalar_x = np.pad(scalar_x, ((0, 0), (0, 0), (0, self.config.MAX_GT_INSTANCES - instance_num)),
                                  mode='constant')
            else:
            ...
            padding = [(0, 0), (0, 0), (0, 0)]
            stride_mask = resize_mask(masks, 0.25, padding, 0)
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            if b >= self.batch_size:
                # print("====batch_data====")
                inputs = (imgs, gt_batch, gt_mask, scalar_y, scalar_x)
                targets = []
                return inputs, targets
            res = self.dataset.generator(image_id=id)
            if res is not None:
                img, gt, mask, sy, sx = res
                imgs.append(img)
                gt_batch.append(gt)
                gt_mask.append(mask)
                scalar_y.append(sy)
