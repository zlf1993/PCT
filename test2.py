...
import numpy as np
import os
from coco.coco import CenterNetTestConfig
...
def ccc(config, num_select, index, select_bbox, exist_i, center, class_seg, num_i, select_lrtb, pic_preg):
    if num_i == 0:
        masks = np.zeros([config.IMAGE_MAX_DIM//config.STRIDE, config.IMAGE_MAX_DIM//config.STRIDE, num_select], np.float32)
    else:
        masks = seg_instance(config, num_select, index, select_bbox, exist_i, center, class_seg, num_i, select_lrtb, pic_preg)
    return masks
...
def seg_instance(config, num_select, index, select_bbox, exist_i, class_seg, num_i, pic_preg):
    ...
    h = range(config.IMAGE_MAX_DIM//config.STRIDE)
    ...
    temp_masks = np.zeros([config.IMAGE_MAX_DIM//config.STRIDE, config.IMAGE_MAX_DIM//config.STRIDE, num_select], np.float32)
...
interpreter = tf.lite.Interpreter(model_path="converted_model.tflite", experimental_delegates=[tf.lite.experimental.load_delegate('libedgetpu.so.1')])
print("=========inter========")
...
interpreter.set_tensor(input_details[0]['index'], tf.constant(image))
print("=========1=========")
interpreter.invoke()
print("=========2=========")
[grid_yx, scores, bbox, class_id, bbox_lrtb, fpn, preg] = interpreter.get_tensor(output_details[0]['index'])
print("=========3=========")
select_center = select_center[0]
select_class_id = select_class_id[0]
select_scores = select_scores[0]
select_bbox = select_bbox[0]
select_lrtb = select_lrtb[0]
...
select_lrtb = select_lrtb[0:num_select, ...]

final_masks = np.zeros([56, 56, num_select], np.float32)
