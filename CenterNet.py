...
class CentLoss(tf.keras.layers.Layer):
    ...
    def _compute_one_image_loss(self, gravity_pred, dist_pred, heatmap_pred, ground_truth, masks, scalar_y, scalar_x,
        ...
        heatmap_pos_loss = - 100 * tf.pow(1. - tf.sigmoid(heatmap_pred), 2.) * \
            tf.math.log_sigmoid(heatmap_pred) * heatmap_gt

        heatmap_neg_loss = - 100 * tf.pow(tf.sigmoid(heatmap_pred), 2.) * \
...
class CenterNet:
    ...
    def _define_inputs(self):
        ...
        if self.mode == 'train':
            ...
            mask_shape = [self.image_size/4, self.image_size/4, self.max_gt_instances]
    ...
    def _fusion_feature(self):
        inter1 = yolo_conv2d(self.s_32, 512, 3, 1)  # /32
        inter1 = yolo_conv2d(inter1, 512, 3, 1)  # /32
        inter1 = self._dconv_bn_activation(inter1, 256, 4, 2)  # /16
        concat1 = tf.concat([inter1, self.s_16], axis=3)  # 256+112=368  /16
        inter2 = yolo_conv2d(concat1, 512, 3, 1)  # /16
        inter2 = yolo_conv2d(inter2, 512, 3, 1)  # /16
        inter2 = self._dconv_bn_activation(inter2, 256, 4, 2)  # /8
        concat2 = tf.concat([inter2, self.s_8], axis=3)  # 256+40=296  /8
        inter3 = yolo_conv2d(concat2, 512, 3, 1)  # /8
        inter3 = yolo_conv2d(inter3, 512, 3, 1)  # /8
        inter3 = self._dconv_bn_activation(inter3, 256, 4, 2)  # /4
        concat3 = tf.concat([inter3, self.s_4], axis=3)  # 256+24=280  /4
        concat3 = yolo_conv2d(concat3, 256, 3, 1)  # /4
